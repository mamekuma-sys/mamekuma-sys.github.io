...
- 현재 로컬 저장소에는 기존 GitHub Pages 저장소의 원격이 연결되어 있지 않다.
- 기존 원격 저장소 `mamekuma-sys/mamekuma-sys.github.io`는 새 사이트가 완성·검증될 때까지 삭제하지 않는다.
- 삭제 직전에 읽기 전용 확인으로 계정·저장소·공개 주소를 다시 검증한다.
- 실제 삭제에는 사용자의 마지막 명시적 확인이 필요하다.
- 삭제·재생성·Pages 전환은 사이트 스캐폴딩과 별도의 후속 계획으로 실행한다.
