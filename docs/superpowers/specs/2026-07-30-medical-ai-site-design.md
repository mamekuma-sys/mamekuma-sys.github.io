...
실제 게시 전 날짜와 평가 값은 조사 결과로 대체한다. 위 예시는 스키마 형태를 설명하기 위한 것이며 사실 주장으로 사용하지 않는다.
...
기존 원격 저장소 `mamekuma-sys/mamekuma-sys.github.io`는 새 사이트가 로컬에서 완성·검증될 때까지 유지한다.

전환 절차는 다음과 같다.

1. 로컬 스캐폴딩과 구현
2. 콘텐츠·이미지 연결
3. UX·시각·접근성·프로덕션 빌드 검증
4. 기존 원격 저장소와 계정 재확인
5. 삭제 직전에 사용자에게 대상과 중단 가능성 최종 고지
6. 사용자 최종 확인
7. 기존 GitHub 저장소 자체 삭제
8. 동일한 이름의 공개 저장소 생성
9. 새 프로젝트의 깨끗한 이력만 `main`에 푸시
10. Actions, Pages, Discussions와 Giscus 설정
11. 실제 주소 최종 검증

기존 원격 저장소의 파일과 Git 이력은 보존하지 않는다. 삭제와 재생성 사이에 사이트가 일시적으로 중단될 수 있다. 정확한 대상을 읽기 전용 명령으로 다시 확인하기 전에는 삭제 명령을 실행하지 않는다.
...
- 기존 원격 저장소 삭제는 새 사이트 검증과 사용자 최종 확인 전에는 실행되지 않는다.
